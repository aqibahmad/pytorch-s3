...
if __name__ == '__main__':
    ...
    # S3 Configuration
    os.environ["AWS_ACCESS_KEY_ID"] = "testuser"
    os.environ["AWS_SECRET_ACCESS_KEY"] = "testuser"
    throughput_target_gbps = 10.0
    part_size = 8 * 1024 * 1024 # => 8MiB
    S3_ENDPOINT = "http://cxo-s3-cluster47.lab.nimblestorage.com"
    ...
    train_dataset = S3MapDataset.from_prefix(TRAIN_DATASET_URI, region=REGION,endpoint=S3_ENDPOINT, s3client_config=s3_config, transform=partial(transform_image, type="train", train_transforms=train_transforms, val_transforms=val_transforms, label_map=label_map))
    val_dataset = S3MapDataset.from_prefix(VAL_DATASET_URI, region=REGION,endpoint=S3_ENDPOINT, s3client_config=s3_config, transform=partial(transform_image, type="val", train_transforms=train_transforms, val_transforms=val_transforms, label_map=label_map))
    ...
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, num_workers=dataload_workers, pin_memory=True, shuffle=True)
    val_dataloader = torch.utils.data.DataLoader(val_dataset, batch_size=batch_size, num_workers=dataload_workers, pin_memory=True, shuffle=False) 
